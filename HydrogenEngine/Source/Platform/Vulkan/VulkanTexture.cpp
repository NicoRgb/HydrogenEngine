...
static void TransitionImageLayout(VkCommandBuffer commandBuffer, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t layerCount=1)
{
	...
	barrier.image = image;
	barrier.subresourceRange.aspectMask = IsDepthFormat(format) ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
...
void VulkanTexture::UploadData(void* data)
{
...

	{
		...
		TransitionImageLayout(commandBuffer, m_Image, m_Format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, m_FinalLayout);
	}
...
void VulkanTexture::CreateTexture()
{
	...
	if (m_FinalLayout != VK_IMAGE_LAYOUT_UNDEFINED)
	{
		VkCommandBufferAllocateInfo cmdAllocInfo{};
		cmdAllocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		cmdAllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
		...
		vkBeginCommandBuffer(commandBuffer, &beginInfo);
		TransitionImageLayout(commandBuffer, m_Image, m_Format, VK_IMAGE_LAYOUT_UNDEFINED, m_FinalLayout);
		vkEndCommandBuffer(commandBuffer);
...
VulkanCubeMap::VulkanCubeMap(const std::shared_ptr<RenderContext>& renderContext, TextureFormat format, size_t width, size_t height, VkImageLayout finalLayout, VkImageUsageFlags usage, VkSampleCountFlagBits samples)
	: m_RenderContext(RenderContext::Get<VulkanRenderContext>(renderContext)), m_Usage(usage), m_Samples(samples), m_FinalLayout(finalLayout)
{
	m_Width = width;
	m_Height = height;
...
void VulkanCubeMap::Resize(size_t width, size_t height)
{
	m_Width = width;
	m_Height = height;
...
void VulkanCubeMap::UploadData(void* data)
{
	size_t size = 6 * m_Width * m_Height * sizeof(uint32_t);
	...
	{
		TransitionImageLayout(commandBuffer, m_Image, m_Format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 6);

		VkBufferImageCopy regions[6];

		for (int i = 0; i < 6; i++)
		{
			regions[i].bufferOffset = m_Width * m_Height * sizeof(uint32_t) * i;
			regions[i].bufferRowLength = 0;
			regions[i].bufferImageHeight = 0;

			regions[i].imageSubresource.aspectMask = IsDepthFormat(m_Format) ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
		...
		TransitionImageLayout(commandBuffer, m_Image, m_Format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, m_FinalLayout, 6);
	}
...
void VulkanCubeMap::CreateTexture()
{
	VkImageCreateInfo imageInfo{};
	imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	imageInfo.imageType = VK_IMAGE_TYPE_2D;
	...
	if (m_FinalLayout != VK_IMAGE_LAYOUT_UNDEFINED)
	{
		...
		vkBeginCommandBuffer(commandBuffer, &beginInfo);
		TransitionImageLayout(commandBuffer, m_Image, m_Format, VK_IMAGE_LAYOUT_UNDEFINED, m_FinalLayout, 6);
		vkEndCommandBuffer(commandBuffer);
